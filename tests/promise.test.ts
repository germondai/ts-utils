...
import { retry, sleep, timeout } from "../src/runtime/promise"
...
describe("timeout", () => {
  test("resolves if the promise completes before the timeout", async () => {
    const result = await timeout(new Promise((resolve) => setTimeout(() => resolve("done"), 10)), 100)
    expect(result).toBe("done")
  })

  test("rejects if the promise takes longer than the timeout", async () => {
    await expect(timeout(new Promise((resolve) => setTimeout(() => resolve("slow"), 200)), 30)).rejects.toThrow(
  ...
  test("uses custom error message when provided", async () => {
    await expect(
      timeout(new Promise((resolve) => setTimeout(() => resolve("slow"), 200)), 30, "Custom timeout message"),
    ).rejects.toThrow("Custom timeout message")
  })

  test("uses default error message when no custom message is provided", async () => {
    await expect(timeout(new Promise((resolve) => setTimeout(() => resolve("slow"), 200)), 50)).rejects.toThrow(
  ...
  test("passes through the rejection of the original promise", async () => {
    await expect(
      timeout(new Promise((_, reject) => setTimeout(() => reject(new Error("original error")), 10)), 200),
  ...
  test("works with immediately rejecting promises", async () => {
    await expect(timeout(Promise.reject(new Error("instant fail")), 100)).rejects.toThrow("instant fail")
  })
})
