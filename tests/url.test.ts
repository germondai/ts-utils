...
import { buildUrl, getQueryParams, removeQueryParam, updateQueryParam } from "../src/runtime/url"
...
describe("getQueryParams", () => {
  ...
  test("handles encoded values", () => {
    const params = getQueryParams("https://example.com/?name=hello%20world&tag=a%26b")
...
describe("updateQueryParam", () => {
  test("updates an existing query parameter", () => {
    const result = updateQueryParam("https://example.com/?search=test", "search", "new")
...
describe("removeQueryParam", () => {
  test("removes an existing query parameter", () => {
    const result = removeQueryParam("https://example.com/?search=test", "search")
...
describe("buildUrl", () => {
  ...
  test("builds URL with mixed param types including null and undefined", () => {
    ...
    expect(result).toBe("https://example.com/api?name=test&count=5&empty=&active=false")
  })
