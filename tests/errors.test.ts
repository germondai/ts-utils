...
describe("catchError", () => {
  ...
  test("catches a TypeError when it is in errorsToCatch", async () => {
    const result = await catchError(() => Promise.reject(new TypeError("type error")), [TypeError])
    expect(result.ok).toBe(false)
  ...
  test("catches error matching one of multiple specified types", async () => {
    const result = await catchError(() => Promise.reject(new RangeError("out of range")), [TypeError, RangeError])
    expect(result.ok).toBe(false)
    expect(result.error).toBeInstanceOf(RangeError)
  })

  test("rethrows errors not in errorsToCatch", async () => {
    expect(catchError(() => Promise.reject(new RangeError("range error")), [TypeError])).rejects.toThrow(RangeError)
  })

  test("rethrows errors when none of the specified types match", async () => {
    expect(catchError(() => Promise.reject(new SyntaxError("syntax")), [TypeError, RangeError])).rejects.toThrow(
      SyntaxError,
    )
  })

  test("catches subclass errors matched by parent class", async () => {
    const result = await catchError(() => Promise.reject(new TypeError("sub")), [Error])
    expect(result.ok).toBe(false)
    expect(result.error).toBeInstanceOf(TypeError)
  })
