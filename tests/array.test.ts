...
describe("sortBy", () => {
  ...
  test("sorts by function", () => {
    ...
    expect(sortBy(items, (s) => s.length)).toEqual(["apple", "banana", "cherry"])
  })
  ...
  test("sorts strings alphabetically by key", () => {
    ...
    expect(sortBy(items, "name")).toEqual([{ name: "Alice" }, { name: "Bob" }, { name: "Charlie" }])
  })
})
...
describe("compact", () => {
  ...
  test("keeps truthy strings and objects", () => {
    expect(compact(["hello", null, { a: 1 }, false])).toEqual(["hello", { a: 1 }])
  })
})
